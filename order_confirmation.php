...
$stmt = $conn->prepare("
    SELECT uoi.product_id, p.name AS product_name, uoi.quantity, uoi.price 
    FROM user_order_items uoi
    JOIN products p ON uoi.product_id = p.product_id
    WHERE uoi.order_id = ?
");
$stmt->bind_param("i", $order_id);
$stmt->execute();
...
<head>
  <title>Supermarketakis - Order Confirmation</title>
  <meta charset="utf-8">
